...
static int aio_port_set_ch(struct uniphier_aio_sub *sub)
{
	struct device *dev = &sub->aio->chip->pdev->dev;
	struct regmap *r = sub->aio->chip->regmap;
	unsigned char *map = sub->aio->chmap;
	u32 slotsel_2ch[] = {
		0, 0, 0, 0, 0,
	};
	u32 slotsel_multi[] = {
	...
	u32 mode, *slotsel;
	u32 chmap_trans_slotset[3][4] = {
		{SNDRV_CHMAP_FL, SNDRV_CHMAP_SL, SNDRV_CHMAP_RL, SNDRV_CHMAP_FC },
		{SNDRV_CHMAP_FR, SNDRV_CHMAP_SR, SNDRV_CHMAP_RR, SNDRV_CHMAP_LFE},
		{OPORTMXTYSLOTCTR_SLOTSEL_SLOT0, OPORTMXTYSLOTCTR_SLOTSEL_SLOT1,
		 OPORTMXTYSLOTCTR_SLOTSEL_SLOT2, OPORTMXTYSLOTCTR_SLOTSEL_SLOT3},
	};
	int i, j, ch, lrrev[5] = {0, 0, 0, 0, 0};

	ch = params_channels(&sub->params);
	for (i = 0; i < ch; i += 2 ) {
		for (j = 0; j < 4; j++) {
			if (map[i] == chmap_trans_slotset[0][j]) {
				if (map[i + 1] == chmap_trans_slotset[1][j]) {
					slotsel_multi[i / 2] = chmap_trans_slotset[2][j];
					lrrev[i / 2] = OPORTMXTYSLOTCTR_LRREV_OFF;
					break;
				} else {
					dev_err(dev, "channel mapping:%d (%d, %d) is invalid\n",
						i, map[i], map[i + 1]);
					break;
				}
			} else if (map[i] == chmap_trans_slotset[1][j]) {
				if (map[i + 1] == chmap_trans_slotset[0][j]) {
					slotsel_multi[i / 2] = chmap_trans_slotset[2][j];
					lrrev[i / 2] = OPORTMXTYSLOTCTR_LRREV_ON;
					break;
				} else {
					dev_err(dev, "channel mapping:%d (%d, %d) is invalid\n",
						i, map[i], map[i + 1]);
	...
	for (i = 0; i < AUD_MAX_SLOTSEL; i++) {
		...
		regmap_update_bits(r, OPORTMXTYSLOTCTR(sub->swm->oport.map, i),
				   OPORTMXTYSLOTCTR_LRREV_MASK, lrrev[i]);
	}
...
void aio_port_set_enable(struct uniphier_aio_sub *sub, int enable)
{
	...
	if (sub->swm->dir == PORT_DIR_OUTPUT) {
		...
		regmap_write(r, IPORTMXIM2(sub->swm->iport.map), IPORTMXIM2_IM31_ON);
 ...
 */
static int aio_port_chmap_ctl_info(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_info *uinfo)
{
	struct snd_pcm_chmap *info = snd_kcontrol_chip(kcontrol);
	struct uniphier_aio_chip *chip = info->private_data;
	struct uniphier_aio *aio = &chip->aios[kcontrol->private_value];

	uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
	uinfo->count = aio->channels_max;
...
static int aio_port_chmap_ctl_get(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	for (i = 0; i < ARRAY_SIZE(aio->chmap); i++)
		ucontrol->value.integer.value[i] = aio->chmap[i];
	return 0;
}

static int aio_port_chmap_ctl_put(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_pcm_chmap *info = snd_kcontrol_chip(kcontrol);
	struct uniphier_aio_chip *chip = info->private_data;
	...
	if (!memcmp(chmap, aio->chmap, sizeof(chmap)))
		return 0;

	memcpy(aio->chmap, chmap, sizeof(chmap));

	return 0;
}

static int aio_port_chmap_ctl_tlv(struct snd_kcontrol *kcontrol, int op_flag,
				  unsigned int size, unsigned int __user *tlv)
{
	...
	size -= 8;
	dst = tlv + 2;
	for (chs = 2; chs <= aio->channels_max; chs++) {

		int chs_bytes = chs * 4;
		int type = SNDRV_CTL_TLVT_CHMAP_VAR;
...
int uniphier_aio_add_chmap_ctls(struct snd_soc_pcm_runtime *rtd)
{
	...
	/* set only playback stream */
	if (!dai->driver->playback.channels_min)
		return 0;

	aio->channels_max = dai->driver->playback.channels_max;
	...
	chmap->private_data = aio->chip;
	kctl = chmap->kctl;
	for (i = 0; i < kctl->count; i++)
		kctl->vd[i].access |= SNDRV_CTL_ELEM_ACCESS_WRITE;
	kctl->info = aio_port_chmap_ctl_info;
	kctl->get = aio_port_chmap_ctl_get;
	kctl->put = aio_port_chmap_ctl_put;
	kctl->tlv.c = aio_port_chmap_ctl_tlv;
