/*
 * ALSA SoC Thru out codec driver.
 *
...
static int thru_out_probe(struct platform_device *pdev)
{
	return snd_soc_register_codec(&pdev->dev, &soc_codec_thru,
		soc_dai_thru, ARRAY_SIZE(soc_dai_thru));
}

static int thru_out_remove(struct platform_device *pdev)
{
...
#ifdef CONFIG_OF
static const struct of_device_id thru_out_of_match[] = {
	{ .compatible = "socionext," DRV_NAME, },
	{}
};
MODULE_DEVICE_TABLE(of, thru_out_of_match);
#endif

static struct platform_driver thru_out_codec_driver = {
	.driver = {
		.name = DRV_NAME,
		.of_match_table = of_match_ptr(thru_out_of_match),
	},
	.probe  = thru_out_probe,
	.remove = thru_out_remove,
};
module_platform_driver(thru_out_codec_driver);

MODULE_AUTHOR("Katsuhiro Suzuki <suzuki.katsuhiro@socionext.com>");
MODULE_DESCRIPTION("Thru out dummy codec driver");
